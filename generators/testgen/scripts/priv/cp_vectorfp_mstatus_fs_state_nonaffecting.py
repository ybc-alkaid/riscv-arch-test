"""Priv coverpoint handler for cp_vectorfp_mstatus_fs_state_nonaffecting.
...
implementation, making "FS did not transition" unobservable in pure
functional coverage. The test sequence is still emitted here so that a
transitioning model can be cross-checked against an always-Dirty model via
...
CP = "cp_vectorfp_mstatus_fs_state_nonaffecting"


@register(CP)
def make_vectorfp_mstatus_fs_state_nonaffecting(instruction: str) -> None:
