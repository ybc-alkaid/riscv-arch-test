...
`define COVER_ZBKC
covergroup Zbkc_clmul_cg with function sample(ins_t ins);
...
endgroup
// ---------------------
covergroup Zbkc_clmulh_cg with function sample(ins_t ins);
    option.per_instance = 0;
    cmp_rd_rs1 : coverpoint ins.get_gpr_reg(ins.current.rd)  iff (ins.current.rd == ins.current.rs1 & ins.trap == 0 )  {
        // Compare assignments of all registers
    }
...
endgroup
// ---------------------
function void zbkc_sample(int hart, int issue, ins_t ins);

    case (traceDataQ[hart][issue][0].inst_name)
        "clmul"     : begin
            Zbkc_clmul_cg.sample(ins);
        end
        "clmulh"     : begin
            Zbkc_clmulh_cg.sample(ins);
        end
    endcase
endfunction
